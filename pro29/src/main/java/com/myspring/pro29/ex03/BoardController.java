...
@RestController // 프론트를 안 만들겠다.
@RequestMapping("/boards") // 처음을 boards로 시작해라
public class BoardController {
	static Logger logger = LoggerFactory.getLogger(BoardController.class);
	
	// /boards/all 을 입력하면 실행
	...
	public ResponseEntity<String> addArticle (@RequestBody ArticleVO articleVO) {
		...
		try {
			...
			resEntity =new ResponseEntity("ADD_SUCCEEDED",HttpStatus.OK); // 성공했을 때 메시지
		}catch(Exception e) {
			resEntity = new ResponseEntity(e.getMessage(),HttpStatus.BAD_REQUEST); // 실패했을 때
		}
