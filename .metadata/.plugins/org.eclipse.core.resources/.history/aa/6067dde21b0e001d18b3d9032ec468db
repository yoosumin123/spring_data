...
public class FileDownloadController {
	 ...
	 * File(CURR_IMAGE_REPO_PATH+"\\"+"thumbnail"+"\\"+fileName+".gif"); if
	 * (image.exists()) { thumbnail.getParentFile().mkdirs();
	 ...
	 * byte[1024 * 8]; while (true) { int count = in.read(buffer); // 버퍼에 읽어들인 문자개수
	 * if (count == -1) // 버퍼의 마지막에 도달했는지 체크 break; out.write(buffer, 0, count); }
	 * in.close(); out.close(); }
	 */
